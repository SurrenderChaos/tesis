...
export default function PerfilReleSegundo() {
 ...
 const handleChange = async (value) => {
    ...
    try {
      await firebase.database().ref('angulo_servo').set(newValue);
    } catch (error) {
  ...
  return (
   <View style={styles.container}>
      <Text style={styles.title}>Perfil Rele 02</Text>
      <Text style={styles.description}>
          Este servomotor corresponde a una ventana
      </Text>
      ...
      <Slider
        ...
        />
      <TouchableOpacity style={styles.button}onPress={() => navigation.navigate("PerfilServoPrimero")}>
        <Text style={styles.buttonText}>Perfil Servo 1</Text>
      </TouchableOpacity>
    </View> 
  );
}
